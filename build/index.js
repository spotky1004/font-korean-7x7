...
const monospace = document.getElementById("monospace-checkbox");
const canvas = new PixelCanvas({
    width: 400,
    height: 400
}, document.getElementById("preview-canvas"));
const editorBlocks = [...document.getElementsByClassName("editor__block")];
...
editorBlocks.forEach((block, i) => block.addEventListener("click", () => {
    ...
    for (let i = 0; i < editorBlocks.length; i++) {
        ...
        if (editorBlocks[i].classList.contains("active"))
            datas.push(`"${x}_${y}"`);
    }
