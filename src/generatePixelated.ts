...
export default function generatePixelated(char: string) {
  char = char[0];
...
function getPixelMatrix(): number[][] {
  return Array(7).fill(null).map(_ => Array(7).fill(0));
}

const shapeTypes = ["c2", "c3", "c5", "v1", "v2", "v3", "h1", "h2"] as const;
function isKoreanShape(shape: (string | -1)[]): shape is (typeof shapeTypes[number])[] {
  return shape.filter(s => !shapeTypes.includes(s as any)).length === 0;
}
function isVaildShape(shape: string): shape is (keyof typeof korData) {
